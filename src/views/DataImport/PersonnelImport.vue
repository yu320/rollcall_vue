...
import { PERSONNEL_IMPORT_HEADERS } from '@/utils/constants';
...
const importFromText = () => {
  let text = manualInput.value.trim();
  if (!text) {
...
const processImport = async (csvText, source) => {
  ...
  importResult.value = null; 
  const validationErrors = []; // 將 validationErrors 移到 try 區塊內
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      let displayLineNumber = source === 'manual' ? i : i + 1;
